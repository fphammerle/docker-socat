...
IMAGE_NAME = docker.io/fphammerle/socat
PROJECT_VERSION = $(shell git describe --match=v* --abbrev=0 --dirty | sed -e 's/^v//')
SOCAT_PACKAGE_VERSION = $(shell grep -Po 'SOCAT_PACKAGE_VERSION=\K.+' Dockerfile | tr -d -)
ARCH = $(shell arch)
...
IMAGE_TAG = ${PROJECT_VERSION}-socat${SOCAT_PACKAGE_VERSION}-${IMAGE_TAG_ARCH}
